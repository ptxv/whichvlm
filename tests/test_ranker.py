...
def test_golden_ocr_for_12gb_prefers_direct_task_model():
    ocr_direct = ModelInfo(
        ...
        gguf_variants=[
            GGUFVariant(
                filename="ocr-Q4_K_M.gguf",
                quant_type="Q4_K_M",
                file_size_bytes=1_800_000_000,
            ),
        ],
    )
    ocr_self_reported = ModelInfo(
    ...
    results = rank_models(
        [general_vlm, ocr_self_reported, ocr_direct],
        make_hardware(vram_gb=12, bandwidth_gbps=450.0),
        top_n=3,
        task_profile="ocr",
...
def test_golden_general_vlm_for_apple_m3_max_prefers_mlx_direct_model():
    mlx_model = ModelInfo(
