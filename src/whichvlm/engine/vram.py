...
def estimate_media_overhead(model: ModelInfo, workload: Workload | None) -> int:
    if workload is None:
        return 0
    ...
    visual = 0
    if visual_inputs > 0 and supports_visual_inputs(model):
    ...
    audio = 0
    if wl.audio_seconds > 0 and supports_audio_inputs(model):
...
def estimate_vram(
    ...
    media = estimate_media_overhead(model, workload)
    framework = FRAMEWORK_OVERHEAD_BYTES
    return weights + kv_cache + activation + media + framework
